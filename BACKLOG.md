...
- Port `WeatherSnapshot` (the 675-line central view model every surface renders from). Its field types are now all ported (`WindDirection`, `UVIndex`, `AirQuality`, `UVHourSlot`, `WeatherAlert`), so the data class + `HourSlot`/`DaySnapshot` + the pure computed helpers (`heroTemp`, `isNight`, `upcomingHours`, `nextSunEvent`, `hasCurrentHourData`, `activeAlert`) can land now. Persisting it needs the same `InstantEpochMillisSerializer` on its own `Date` fields (`sunrise`, `sunset`, `updated`). The `make()` factory still waits on the fetch-side types (`Location`, `StationObservation`, `ForecastBulletin`), so it lands with the client/repository step; the `ActiveAlertTests` and `AirComponentsTests` Swift cases that gate `activeAlert`/the MITECO client port then too.
