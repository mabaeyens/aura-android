...
Play also warns that the app uses APIs or parameters deprecated in Android 15 for edge-to-edge (`Window.setStatusBarColor`, `Window.setNavigationBarColor`, `LAYOUT_IN_DISPLAY_CUTOUT_MODE_SHORT_EDGES`). That too is a recommendation, not a blocker, and it is not Aura's code: all three live inside androidx.activity's `enableEdgeToEdge()` compat path, which `MainActivity` calls once. The call sites Play names (`com.airbnb.lottie...ImageLayer.getBitmap`, `b.w.b`, `b.u.v`) are R8 inlining artifacts, not the real origin, so Lottie is a red herring. It cannot be fixed app-side without a regression: on minSdk 26 through API 34 those deprecated calls are the only supported way to get transparent bars, so hand-rolling edge-to-edge would either reintroduce the identical calls in our own code or drop transparent bars on most in-use devices. The warning is harmless (the app is genuinely edge-to-edge correct, and the setters are no-ops on Android 15) and clears when AndroidX updates the library; the only real lever is bumping `activityCompose` in `gradle/libs.versions.toml` once a fixed release ships, as its own reviewed change. Safe to dismiss on every upload.
